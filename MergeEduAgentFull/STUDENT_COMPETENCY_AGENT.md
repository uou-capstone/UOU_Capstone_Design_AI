...
교사가 강의실 전체 또는 참여 학생별 누적 리포트를 볼 수 있게 만드는 기능이다.
...
저장 키는 `classroomId + reportScope + studentUserId` 조합이다.
...
- `JsonStore`는 process-local file/session lock과 unique temp atomic write를 사용하지만, 여러 JSON 파일을 묶는 DB transaction을 제공하지는 않는다.
...
1. 강의실의 모든 `week -> lecture -> session`을 수집한다.
   - lecture 목록은 `listLecturesByWeekIds(...)`로 한 번에 읽어 week id별로 그룹핑한다.
...
서버는 단계 이벤트와 delta를 보낸다.
현재 UI는 진행 바와 `thought_delta` 중심의 사고 진행 상황을 표시하며, `answer_delta`는 서버 계약에 남아 있지만 화면 누적 표시에는 사용하지 않는다.

### 3.3 학생별 리포트 목록과 분석 실행

`GET /api/classrooms/:classroomId/report/students`

- 초대된 학생 목록과 저장된 학생별 리포트 요약을 반환한다.
- 저장된 리포트가 있으면 `overallScore`, `overallLevel`, `analysisStatus`, `generationMode`, `sourceStats` 요약을 포함한다.
- 저장된 리포트가 없어도 `enrichStudentProgressStats(...)`로 학생별 진행 페이지 수와 진도 비율을 최신 세션 기준으로 보강한다.

`GET /api/classrooms/:classroomId/report/students/:studentUserId`

- 특정 학생의 저장된 리포트를 반환한다.
- 학생이 해당 강의실에 초대되어 있고 이메일 인증이 완료된 학생 계정일 때만 조회된다.

`POST /api/classrooms/:classroomId/report/students/:studentUserId/analyze/stream`

- 특정 학생의 세션만 집계해 학생별 리포트를 생성하고 저장한다.
- 내부적으로 `aggregateClassroomSource(..., { ownerIds: [student.id], reportScope: "STUDENT" })`를 사용한다.
- Gemini 호출과 heuristic fallback 병합 규칙은 강의실 전체 리포트와 동일하되, `reportScope: "STUDENT"`와 `studentUserId`가 보존된다.

학생별 API는 모두 선생님 계정, 강의실 소유자, 이메일 인증 완료 상태를 요구한다.
...
- 최근 피드백은 `week · lecture p.page:` 위치 prefix를 붙이고, 피드백 원문은 120자 이내로 정규화/절단한다.
...
현재 세션은 `ownerUserId`를 가진다.

- 학생별 리포트는 해당 학생의 `ownerUserId`와 일치하는 세션만 읽는다.
- 강의실 전체 리포트는 초대된 학생이 있으면 초대 학생들의 owner session을 합산한다.
- 초대 학생이 없는 과거/레거시 강의실은 선생님 소유 세션 또는 legacy unowned session을 보조적으로 읽는다.

따라서 한 강의실 안에서 학생 A와 학생 B가 같은 강의를 들어도 질문, 퀴즈, 피드백, 메모리는 리포트 입력에서 분리된다.

### 4.7 assessment artifact와의 관계
...
- 같은 강의실 전체 리포트가 있으면 `CLASSROOM_AGGREGATE` 범위 안에서 교체된다.
- 같은 학생별 리포트가 있으면 `STUDENT + studentUserId` 범위 안에서 교체된다.
- 강의실 전체 저장본은 `GET /api/classrooms/:classroomId/report`에서 다시 불러온다.
- 학생별 저장본은 `GET /api/classrooms/:classroomId/report/students/:studentUserId`에서 다시 불러온다.
...
- 학생별 리포트는 `ownerUserId`가 기록된 세션을 기준으로 분리된다. 다만 과거 legacy unowned session은 강의실 전체 리포트의 보조 입력으로만 남는다.
...
`강의실/학생별 세션 원시 데이터 집계 -> 로컬 heuristic 초안 -> Gemini 해석 보강 -> 범위별 저장본 재사용`
구조로 보는 것이 가장 정확하다.
