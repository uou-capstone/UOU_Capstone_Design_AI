...
test("teacher classroom uses local navigation sections", async ({ page }) => {
  ...
  const reportNav = page.getByTestId("classroom-nav-report");
  ...
  await expect(page.getByText("초대 학생 #2468")).toBeVisible();
  ...
  await expect(reportNav).toHaveAttribute("aria-current", "page");
  await expect(page.getByTestId("classroom-report-entry")).toBeVisible();
  await expect(page.getByTestId("classroom-week-timeline")).toHaveCount(0);
  await expect(page.getByTestId("classroom-invite-panel")).toHaveCount(0);
  expect(state.getStudentsRequestCount()).toBe(studentsRequestCountOnInvite);
  await expect(page.getByRole("link", { name: "학생 리포트 보기" })).toHaveAttribute(
    "href",
    "/classrooms/cls_nav/report"
  );
...
test("classroom local navigation fits mobile width", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "세부 강의 추가" }).click();
  await expect(page.getByRole("dialog", { name: "강의 자료 업로드" })).toBeVisible();
  await page.getByTestId("classroom-nav-report").click();
  await expect(page.getByRole("dialog", { name: "강의 자료 업로드" })).toHaveCount(0);
  await expect(page.getByTestId("classroom-nav-report")).toHaveAttribute("aria-current", "page");
  ...
  for (const testId of ["classroom-section-nav", "classroom-section-panel"]) {
