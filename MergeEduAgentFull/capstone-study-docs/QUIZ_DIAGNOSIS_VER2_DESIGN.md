...
최신 전체 오케스트레이션 계약은 `AGENT_ORCHESTRATION_DESIGN.md`를 우선 기준으로 본다.
...
- 전용 bridge endpoint가 아니라 `answerQuestionStream`을 교정 prompt로 재사용
- 교정 설명 뒤 재확인 위젯으로 연결
- 런타임 메시지/stream label은 별도 repair label이 아니라 `EXPLAINER`로 기록
...
2026-04-25 기준으로 세션 이벤트 처리와 protected save는 같은 session id 기준 lock 안에서 수행된다.
또한 JSON 파일 read-modify-write에는 process-local file lock과 unique temp atomic write가 적용된다.
다만 여러 JSON 파일을 묶는 DB transaction 수준의 원자성은 제공하지 않는다.

### 7.5 MCQ/OX fast path와 진단 흐름

pending assessment가 없고 제출된 quiz id로 저장된 `QuizRecord`가 존재하며
저장된 `quizRecord.quizType`이 `MCQ` 또는 `OX`이면,
현재 엔진은 LLM planner와 PDF page context를 건너뛰고 deterministic fallback plan으로 즉시 `AUTO_GRADE_MCQ_OX`를 실행한다.

이 fast path는 진단/교정 레이어를 우회하지 않는다.
채점 결과가 기준 미달이면 기존처럼 `QuizDiagnosisService`가 `activeIntervention`을 만들고,
학생의 다음 `USER_MESSAGE`가 교정 답변 흐름으로 이어진다.
