...
- pending assessment가 없는 MCQ/OX 제출은 LLM planner와 PDF page context를 건너뛰는 deterministic fast path를 사용한다.
...
- 세션 이벤트 처리와 protected save는 같은 session id 기준 process-local lock 안에서 실행된다.
- 세션 스트림 연결 종료는 `AbortSignal`로 서버 fetch와 하위 agent stream까지 전파된다.
...
`handleEvent(...)`, `handleEventStream(...)`은 먼저 `JsonStore.withSessionLock(sessionId, ...)`로
같은 세션의 이벤트를 직렬화한다. 따라서 아래 순서는 같은 session id lock 안에서 실행된다.

현재 구현의 일반 경로 실제 순서는 아래와 같다.
...
6. pending assessment가 없는 MCQ/OX 제출이면 deterministic fast path로 이동한다.
7. 일반 경로에서는 `PdfIngestService.readPageContext(...)`로 현재/이전/다음 페이지 텍스트를 읽는다.
8. `planWithLlm(...)`로 오케스트레이터 plan을 만든다.
9. `normalizePlan(...)`으로 `pedagogyPolicy`가 항상 존재하도록 보정한다.
10. plan의 `memoryWrite`가 있으면 `applyLearnerMemoryWrite(...)`로 세션 메모리를 병합한다.
11. plan 안에 퀴즈 생성 tool이 있으면 quiz context를 추가 준비한다.
12. `ToolDispatcher.dispatch(...)`로 tool action을 순차 실행한다.
13. 오케스트레이터 thought summary를 ORCHESTRATOR 메시지에 주입한다.
14. `SummaryService.summarize(...)`로 최근 대화 요약을 다시 만든다.
15. handoff에 사용한 assessment가 있으면 `markAssessmentsConsumed(...)`로 `CONSUMED` 처리한다.
16. 새로 채점된 퀴즈가 있으면 `quiz-results.json` 로그를 append 한다.
17. 최종 세션을 저장하고 `newMessages + ui + patch`를 반환한다.

stream 경로에서 `AbortSignal`이 이미 aborted 상태이거나 planner/dispatch 이후 aborted 상태가 되면
요청은 중단되고 stale final 저장 또는 `final/error` emit을 하지 않는다.
...
- deterministic MCQ/OX fast path 조건이 아님
...
### 4.3 MCQ/OX deterministic fast path

아래 조건을 모두 만족하면 `OrchestrationEngine`은 PDF page context와 LLM planner를 건너뛴다.

- 이벤트가 `QUIZ_SUBMITTED`
- 제출된 quiz id가 세션의 저장된 `QuizRecord`에 존재함
- 저장된 `quizRecord.quizType`이 `MCQ` 또는 `OX`
- 이번 턴에 넘겨야 할 pending assessment handoff가 없음

이 경로는 빈 page context로 `Orchestrator.fallback(...)`을 호출해 `AUTO_GRADE_MCQ_OX` plan을 만들고,
`ToolDispatcher`가 서버 내부 정답 비교만으로 채점한다.
pending assessment가 있으면 assessment handoff를 유지해야 하므로 기존 LLM planner 경로를 탄다.
...
- `HOLD_BACK`, `MINIMAL_HINT`, `allowDirectAnswer=false`, `EXPLAIN_FIRST` 같은 정책별 보정
- `interventionBudget`와 전체 action 하드캡 적용
- `[verifier]` 로그 출력

현재 inline verifier의 실제 규칙 순서는 `R1 -> R7 -> R8 -> R6 -> R9 -> R5 -> R4 -> R2 -> R3 -> R10`이다.
`CHECK_READINESS`, `SRL_REFLECTION`은 prompt 정책 모드에는 존재하지만 별도 전용 verifier rule은 아직 없다.
LLM prompt는 action 4개 이하를 요구하지만, dispatcher의 최종 hard cap은 8개다.

즉, plan 안전성은 `Orchestrator prompt 제약 + ToolDispatcher verifier` 이중 방어로 유지된다.

### 6.2 AI tool 실행 전 공통 처리

- `AUTO_GRADE_MCQ_OX`는 deterministic tool이므로 `geminiFile`과 page context guard 이전에 실행한다.
- 그 외 AI tool은 대상 lecture의 `geminiFile`이 없으면 SYSTEM 메시지를 추가한다.
...
- 서버 내부 정답 비교로 채점
- 클라이언트 payload의 `quizType`이 아니라 세션에 저장된 `quizRecord.quizType`만 신뢰한다.
- 저장된 quiz가 MCQ/OX가 아니면 SYSTEM 메시지를 추가하고 deterministic 채점을 중단한다.
- OX는 공백/미응답을 `미응답입니다.`로 처리한다.
- page 상태 변경은 현재 page가 아니라 `quizRecord.createdFromPage` 기준으로 수행한다.
...
- `thoughtSummary` 반환
- stream 계열 메서드는 `(input, onDelta?, signal?)` 형태로 `AbortSignal`을 받을 수 있다.
...
- `clientContext.currentPage`가 있으면 reducer 초입에서 먼저 반영하고 `lastTouchedAt`을 갱신
- user message append
- next 명령 감지 시 `currentPage += 1`
- `PAGE_CHANGED` 반영
- `SESSION_ENTERED`에서 완료된 페이지는 `DONE -> EXPLAINED`로 되돌려 재학습 가능하게 처리
- `START_EXPLANATION_DECISION` accept 시 `NEW/DONE -> EXPLAINING`, reject 시 `EXPLAINING -> NEW`
- `QUIZ_DECISION`, `QUIZ_TYPE_SELECTED`, `REVIEW_DECISION`, `RETEST_DECISION`에 따른 기본 page status 변경
- `NEXT_PAGE_DECISION` accept 시 `fromPage + 1` 기준으로 이동
- `QUIZ_SUBMITTED`는 실제 채점 전 page 상태를 `QUIZ_GRADED`로 바꾸지 않고 `lastTouchedAt`만 갱신
...
sequenceDiagram
  ...
  E->>E: acquire session lock
  E->>E: StateReducer.reduce
  E->>QA2: preparePendingAssessmentHandoff
  alt MCQ/OX and no pending assessment
    E->>O: fallback plan without PDF page context
  else normal planner path
...
`POST /api/session/:sessionId/event/stream`은 request close 시 `AbortController`를 abort한다.
이 signal은 `OrchestrationEngine`, `GeminiBridgeClient.streamBridge(...)`,
그리고 하위 agent stream 메서드로 전달된다.
단, signal은 서버 fetch 제어용이며 AI bridge JSON payload에는 포함하지 않는다.
abort 후에는 추가 delta, `final`, `error` emit을 하지 않는다.

### 10.2 Gemini 기능 사용

현재 구현은 다음 Gemini 기능을 사용하되 적용 범위가 다르다.
...
non-stream bridge endpoint는 주로 직접 file part를 전송한다.
...
- `POST /api/session/:sessionId/save`
  ...
  - 같은 session id lock 안에서 최신 서버 세션을 다시 읽고 저장
- `POST /api/session/:sessionId/event`
  - 단건 이벤트 처리
- `POST /api/session/:sessionId/event/stream`
  - NDJSON 스트리밍 처리
  - request close 시 세션 이벤트 stream에 한해 abort 전파
...
`final.data.patch`는 현재 아래 필드를 포함한다.
...
- `learnerModel`
- `activeIntervention`
- `quizRecord`: `QUIZ_SUBMITTED`에서 제출 quiz id가 있으면 갱신된 `QuizRecord` 또는 `null`, 그 외 이벤트에서는 `undefined`
...
- 저장 일관성
  - `JsonStore`는 process-local file/session lock과 `.tmp -> rename` 방식의 atomic write를 사용
  - temp path는 pid/time/random 기반으로 유니크하게 생성
  - cascade delete 같은 다중 파일 변경은 DB transaction 수준의 원자성을 보장하지 않는다.
