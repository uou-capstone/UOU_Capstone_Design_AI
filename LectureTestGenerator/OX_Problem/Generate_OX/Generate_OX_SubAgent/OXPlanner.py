...
def OXPlanner(lecture_material, user_profile, target_count):
    # 환경 변수 불러오기
    ...
    ox_planner_schema_path = find_file("OXPlanner_Object.json", base_path)
    ox_planner_system_prompt_path = find_file("OXPlanner_SystemPrompt.md", base_path)

    if not ox_planner_schema_path or not ox_planner_system_prompt_path:
        raise FileNotFoundError("OX Planner 프롬포트 파일을 찾을 수 없습니다")

    with open(ox_planner_schema_path, 'r', encoding='utf-8') as f:
        ox_planner_schema = json.load(f)

    with open(ox_planner_system_prompt_path, 'r', encoding='utf-8') as f:
        ox_planner_system_prompt = f.read()
    ...
    response = client.models.generate_content(
        ...
        contents = ox_planner_contents,
        config={
            "system_instruction": ox_planner_system_prompt,
            "response_mime_type": "application/json",
            "response_schema": ox_planner_schema
        })
