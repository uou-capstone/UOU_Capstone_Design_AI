...
def generate_question(missing_fields, lecture_material):
    # 환경 변수 불러오기
    ...
    # Debate_Agent/phase1/phase1_subAgent/Generate_Question.py -> 3단계 위가 Debate_Agent 폴더
    base_path = os.path.dirname(os.path.dirname(os.path.dirname(current_file_path)))

    question_object_schema_path = find_file("question_Object_Schema.json", base_path)
    question_system_prompt_path = find_file("Generate_Question_System_Prompt.md", base_path)

    if not question_object_schema_path or not question_system_prompt_path:
        raise FileNotFoundError("Generate Question 프롬포트 파일을 찾을 수 없습니다")

    with open(question_object_schema_path, 'r', encoding='utf-8') as f:
        question_object_schema = json.load(f)

    with open(question_system_prompt_path, 'r', encoding='utf-8') as f:
        question_system_prompt = f.read()
    ...
    # missing_fields를 딕셔너리에서 -> 문자열로 바꾼다
