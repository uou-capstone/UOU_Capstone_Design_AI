...
def run_debate_session_logic(session_profile, lecture_material):
    """
    토론 세션을 진행하는 Phase 2 함수
    
    Args:
        session_profile: Phase 1에서 생성된 세션 설정
        lecture_material: 강의 자료 경로 또는 텍스트
        
    Returns:
        tuple: (final_status, evaluation_logs, summary_context, history)
    """
    # 1. 설정값 파싱
    ...
    print(f"=== [{dialogue_style}] 세션을 시작합니다. (난이도: {difficulty}) ===")
