...
## Flash_Card 프로젝트 구조 개요

```
Flash_Card/
├── Flash_Card_Main.py                  # 메인 엔트리 포인트 (워크플로우 조율)
├── Flash_card_Main_Guide.md            # 현재 문서 (워크플로우 가이드)
├── Flash_card_main_Example.text        # 실행 예시 로그
│
├── Flash_Card_Main_test/               # 메인 워크플로우 테스트
│   └── Flash_Card_Main_test.ipynb
│
└── Generate_flashCard/                 # 플래시 카드 생성 모듈
    ├── Generate_flashCard.py           # 카드 생성 메인 로직 (Planner → Writer → Validator 루프)
    └── Gen_flashCard_SubAgent/         # 카드 생성 하위 에이전트들
        ├── ConceptPlanner.py           # 핵심 개념 추출 및 출제 계획 수립 에이전트
        ├── CardWriter.py               # 실제 플래시 카드 작성 에이전트 (앞면/뒷면)
        ├── QualityValidator.py         # 품질 검증 에이전트
        │
        ├── Gen_flashCard_Prompt/       # 각 에이전트 프롬프트 및 스키마
        │   ├── ConceptPlannerPrompt/
        │   │   ├── ConceptPlanner_Object.json
        │   │   └── ConceptPlanner_SystemPrompt.md
        │   ├── CardWriterPrompt/
        │   │   ├── CardWriter_Object.json
        │   │   └── CardWriter_SystemPrompt.md
        │   └── QualityValidatorPrompt/
        │       ├── QualityValidator_Object.json
        │       └── QualityValidator_SystemPrompt.md
        │
        └── Gen_flashCard_Test/         # 각 에이전트 개별 테스트
            ├── ConceptPlanner_Test.ipynb
            ├── CardWriter_test.ipynb
            └── QualityValidator_test.ipynb
```

**주요 특징**:
- **카드 생성 파이프라인**: ConceptPlanner(개념 추출) → CardWriter(카드 작성) → QualityValidator(검증)의 3단계 품질 보증 시스템
- **앞면/뒷면 구조**: 각 플래시 카드는 질문(Front)과 답변(Back)으로 구성
- **Common 모듈 의존**: 사용자 프로필 생성(`Prior_Profile_Gen_Agent`) 및 통합 피드백(`generate_FeedBack`)은 공통 모듈 사용
- **플래시 카드 특화**: OX나 5지선다와 달리 채점/피드백 단계가 없으며, 생성 단계만 존재
